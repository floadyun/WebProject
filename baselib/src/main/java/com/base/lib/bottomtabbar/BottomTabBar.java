...
import com.base.lib.bottomtabbar.custom.CustomFragmentTabHost;
import com.base.lib.bottomtabbar.util.TintUtil;
...
public class BottomTabBar extends LinearLayout {
    ...
    public BottomTabBar(Context context, AttributeSet attrs) {
        ...
        if (attributes != null) {
            ...
            //分割线高度
            dividerHeight = attributes.getDimension(R.styleable.BottomTabBar_tab_divider_height, dp2px(0.5f));
            //是否显示分割线
    ...
    public BottomTabBar addTabItem(String name, int imgIdSelect, int imgIdUnSelect, Class fragmentClass) {
        return addTabItem(name, ContextCompat.getDrawable(context, imgIdUnSelect), ContextCompat.getDrawable(context, imgIdSelect), fragmentClass);
    }
    ...
    private View getTabItemView(String name, Drawable drawable) {
        ...
        //设置属性 避免不居中的情况
        LayoutParams ivParams = new LayoutParams(-1, -1);
        ivParams.topMargin = (int) paddingTop;
        ivParams.gravity = Gravity.CENTER;
        ivParams.weight = 1;
        view.setLayoutParams(ivParams);
...
//        tv.setTextSize(fontSize);
        ...
        states[2] = new int[]{};
        int[] colors = new int[]{unSelectColor, selectColor, unSelectColor};//colorSelect跟states[0]对应，color跟states[2]对应，以此类推
